package nl.thecheerfuldev.mymod.tileentity;
...
import nl.thecheerfuldev.mymod.util.helpers.NBTHelper;
...
public class QuarryTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    public void read(final CompoundNBT compound) {
        super.read(compound);
        final CompoundNBT initValues = compound.getCompound("initvalues");
        if (initValues != null) {
    ...
    private void execute() {
        int index = 0;
        final Block[] blocksRemoved = new Block[9];
        for (int x = 0; x < 3; x++) {
            for (int z = 0; z < 3; z++) {
                final BlockPos posToBreak = new BlockPos(this.x + x, this.y, this.z + z);
    ...
    private boolean destroyBlock(final BlockPos pos, final boolean dropBlock, @Nullable final Entity entity) {
        ...
        if (blockState.isAir(world, pos)) {
            ...
            if (dropBlock) {
                final TileEntity tileEntity = blockState.hasTileEntity() ? world.getTileEntity(pos) : null;
